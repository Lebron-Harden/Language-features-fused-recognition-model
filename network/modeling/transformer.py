...
class WrapDecoder(nn.Layer):
    ...
    def __init__(self,
        ...
        self.prepare_decoder = PrepareDecoder(
            ...
            prepostprocess_dropout,
            bos_idx=bos_idx)
        self.encoder = Decoder(n_layer, n_head, d_key, d_value, d_model,
                               d_inner_hid, prepostprocess_dropout,
                               attention_dropout, relu_dropout, preprocess_cmd,
                               postprocess_cmd)

    def forward(self, enc_inputs):
        src_word, src_pos, src_slf_attn_bias, middle_feature = enc_inputs
        if src_word.shape == middle_feature.shape:
            middle_feature = self.prepare_decoder(middle_feature, src_pos)

        enc_input = self.prepare_decoder(src_word, src_pos)

        enc_output = self.encoder(enc_input, src_slf_attn_bias, middle_feature)
        return enc_output
...
class Decoder(nn.Layer):  
    ...
    def forward(self, enc_input, attn_bias, middle_feature):
        enc_output = enc_input
        for i, encoder_layer in enumerate(self.encoder_layers):
            enc_output = encoder_layer(enc_input, attn_bias=attn_bias, middle_feature=middle_feature)
...
class DecoderLayer(nn.Layer): 
    ...
    def forward(self, enc_input, attn_bias=None, middle_feature=None):
        if middle_feature is None:
            ...
            attn_output = self.self_attn(
                    self.preprocesser1(enc_input), self.preprocesser1(middle_feature), self.preprocesser1(middle_feature), attn_bias=attn_bias)
            attn_output = self.postprocesser1(attn_output, enc_input)  
            ffn_output = self.ffn(self.preprocesser2(attn_output))   
            ffn_output = self.postprocesser2(ffn_output, attn_output) 
...
class MultiHeadAttention(nn.Layer): 
    ...
    def __init__(self, d_key, d_value, d_model, n_head=1, dropout_rate=0.):
        ...
        self.q_fc = paddle.nn.Linear(
            in_features=d_model, out_features=d_key * n_head, bias_attr=False) # x * Wq = Q
        self.k_fc = paddle.nn.Linear(
            in_features=d_model, out_features=d_key * n_head, bias_attr=False) # x * Wk = K
        self.v_fc = paddle.nn.Linear(
            in_features=d_model, out_features=d_value * n_head, bias_attr=False) # x * Wv = V
    ...
    def _prepare_qkv(self, queries, keys, values, cache=None):
        ...
        q = paddle.transpose(x=q, perm=[0, 2, 1, 3]) # [0, self.n_head, 0, self.d_key]
        ...
        if cache is not None and static_kv and "static_k" in cache:
            ...
            k = paddle.transpose(x=k, perm=[0, 2, 1, 3]) # [0, self.n_head, 0, self.d_key]
            v = paddle.reshape(x=v, shape=[0, 0, self.n_head, self.d_value])
            v = paddle.transpose(x=v, perm=[0, 2, 1, 3]) # [0, self.n_head, 0, self.d_value]
    ...
    def forward(self, queries, keys, values, attn_bias=None, cache=None):
        # compute q ,k ,v
