...
def load_dygraph_params(config, model, logger, optimizer):
    ...
    if ckp and os.path.exists(ckp + ".pdparams"):
        ...
        '''head_params = paddle.load('./output/rec_res18_transformer_visual/best_accuracy.pdparams')
        
        for k0 in head_params.keys():
            if 'backbone' in str(k0):
                para.append(k0)

        logger.info("\n\ndidn't load the parameters of backbone\n\n")
        for k1 in state_dict.keys():
            #if not 'backbone' in str(k1):
            state.append(k1)
        for k2 in params.keys():
            if not 'backbone' in str(k2):
                para.append(k2)
